#include "hal.h"
// #include "/home/victorgilbert/Portage_UCX-OS_TMS570/repo/ucx-os-forked/include/ucx.h"
...
{
	return jet_system_time() / 1000000; //Cette fonction retourne en ns et on retourne ne ms
}
...
void _context_init(struct jmp_buf *ctx, size_t sp, size_t ss, size_t ra)
{
	/*
		...
		if (cfg->pinsel & (1 << i)) {
			...
			switch (modesel) {
				...
				case GPIO_OUTPUT:
					...
					switch (cfg->port) {
						case GPIO_PORTA:  gioPORTA->DIR &= ~(uint32)((uint32)1 << i);break;
						case GPIO_PORTB:  gioPORTB->DIR &= ~(uint32)((uint32)1 << i);break;
						default: return -1;
					}
				case GPIO_OUTPUT_OD:
...
long pwm_ll_setup(struct pwm_config_values_s *cfg){return -1;}
