...
INC_DIRS = -I $(ARCH_DIR)\
	-I $(ARCH_DIR)/../../riscv/hf-riscv/drivers
...
CFLAGS = -Wall -O2 -c -mips1 -fno-delayed-branch -mno-check-zero-division -msoft-float -ffreestanding -nostdlib -fomit-frame-pointer -G 0 $(INC_DIRS) -DF_CPU=${F_CLK} -D USART_BAUD=$(SERIAL_BAUDRATE) -DF_TIMER=${F_TICK} -DF_TIMER_FIXED="${F_TICK_FIXED}" -DBIG_ENDIAN $(CFLAGS_STRIP) -DDEBUG_PORT
ARFLAGS = r
...
hal:
	...
	$(CC) $(CFLAGS) \
		$(ARCH_DIR)/../../riscv/hf-riscv/hal.c \
		$(ARCH_DIR)/../../riscv/hf-riscv/interrupt.c \
		$(ARCH_DIR)/../../common/muldiv.c \
		$(ARCH_DIR)/../../common/ieee754.c \
		$(ARCH_DIR)/../../common/math.c \
		$(ARCH_DIR)/../../riscv/hf-riscv/drivers/usart.c \
		$(ARCH_DIR)/../../riscv/hf-riscv/drivers/gpio_ll.c \
		$(ARCH_DIR)/../../riscv/hf-riscv/drivers/pwm_ll.c
